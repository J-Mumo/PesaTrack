...
| **M-PESA SMS Parser** | [`MpesaSmsParser.kt`](../android/app/src/main/java/com/pesatrack/utils/parsers/MpesaSmsParser.kt:31) | 9 M-PESA expense types |
...
| Pochi la Biashara | [`MpesaSmsParser.kt`](../android/app/src/main/java/com/pesatrack/utils/parsers/MpesaSmsParser.kt) | Pattern: `"sent to MERCHANT on DATE"` (no phone, no account) — classified as `BUY_GOODS` |
...
- **AI Pro Phase 1 — backend live + Android foundation (v1.6.0 in flight, `feat/ai-pro-plan` branch)** — Two-part work-in-progress migrating from the never-shipped Daraja backend to the PesaTrack Pro subscription + AI Coach architecture from [plans/ai-pro-phase1-spec.md](../plans/ai-pro-phase1-spec.md). **Part 1 — Backend deployed live:** Node/Express + Prisma + Postgres 16 service at `https://pesatrack-api.jmumo.com` (Hetzner CPX21, behind the shared Caddy proxy that already fronts `stockup.jmumo.com` / `maliscope.jmumo.com` / `pesatrack.jmumo.com`; TLS via Let's Encrypt). Daraja code moved to `backend/src/legacy/` (git-mv, history preserved) before the new scaffold landed. Endpoints: `GET /health`, `POST /billing/verify` (validates Google Play purchase token → normalized entitlement → upsert into `entitlements` table keyed on SHA-256(token)), `GET /billing/entitlement` (Bearer-guarded lookup), `POST /ai/echo` (Phase-1 pipeline proof, no LLM call), plus explicit 501 for other `/ai/*` paths until Phase 2 ships. Security invariants **verified live** via a 10-test smoke suite from a laptop against the public URL: purchase tokens only stored as SHA-256 hex (never raw), only the first 16 chars in logs, no request bodies logged, request-id middleware propagates end-to-end (Caddy → Express → response header), CORS scoped to the website origin, and a config-loader guard hard-fails boot on `DEV_FAKE_BILLING=true && NODE_ENV=production`. First deploy exposed two real Dockerfile bugs that got fixed in-flight: Prisma's schema engine mis-detects OpenSSL on Alpine musl (fix: `node:20-alpine` → `node:20-slim` + explicit apt install of `openssl ca-certificates wget`; also fixed `COPY package.json` → `COPY package*.json` so `npm ci` actually runs against the committed lockfile), and the generated Prisma client at `src/generated/prisma/` was stranded in the deps build stage because that path is gitignored (fix: `COPY --from=deps /app/src/generated ./src/generated` in the runtime stage). Deployed with `DEV_FAKE_BILLING=false` + no Play service-account mounted yet, so `/billing/verify` correctly returns 500 on real purchase tokens (the config guard proof) — the Play SA gets mounted at Slice A4 time. **Part 2 — Android Slice A1 (foundation, no user-visible change):** Added Gradle deps for the whole Phase-1 client surface — `com.android.billingclient:billing-ktx:7.1.1` (subscription IAP with `ProductDetails` API), `com.squareup.okhttp3:okhttp:4.12.0` + `logging-interceptor` (HTTP client), `com.squareup.retrofit2:retrofit:2.11.0` + `converter-moshi:2.11.0` + `com.squareup.moshi:moshi:1.15.1` + `moshi-kotlin:1.15.1` + `moshi-kotlin-codegen:1.15.1` via KSP (JSON de/serialization with codegen — no reflection on the hot path). New [network_security_config.xml](../android/app/src/main/res/xml/network_security_config.xml) whitelists `pesatrack-api.jmumo.com` under `cleartextTrafficPermitted=false` with system trust anchors only (no user CAs, no debug overrides — same policy in release and debug so cert misconfigs surface during development); wired via `android:networkSecurityConfig="@xml/network_security_config"` on the `<application>` tag in [AndroidManifest.xml](../android/app/src/main/AndroidManifest.xml). `INTERNET` + `ACCESS_NETWORK_STATE` permissions were already in the manifest from v1.5.0 telemetry, so no new user-visible permission ask. **Part 3 — Android Slice A2 (persisted Pro state, no UI):** Introduced the client-side representation of a PesaTrack Pro entitlement and a single atomic DataStore write path. Three new source files under a new `services/pro/` package. [ProProduct.kt](../android/app/src/main/java/com/pesatrack/services/pro/ProProduct.kt) is an enum `{ MONTHLY("pesatrack_pro_monthly"), ANNUAL("pesatrack_pro_annual") }` where the `productId` strings are the durable contract with both the Play Console SKU configuration and the backend's `KNOWN_PRODUCT_IDS` set (`backend/src/services/playBilling.js`). The enum values also carry a Moshi `@Json(name = "monthly" | "annual")` short label that becomes the wire format inside the persisted JSON — deliberately decoupled from the Kotlin identifier so a rename can't silently break older installs. A `fromProductId(String?)` reverse lookup returns `null` for unknown ids so callers pick between "log-and-drop" vs "fail loudly" per site. [ProState.kt](../android/app/src/main/java/com/pesatrack/services/pro/ProState.kt) is the `@JsonClass(generateAdapter = true)` data class `(isEntitled: Boolean, tier: ProProduct?, purchaseToken: String?, expiresAtEpochMs: Long?, lastVerifiedAtEpochMs: Long?, autoRenewing: Boolean)` with a `DEFAULT` singleton for fresh installs. KDoc spells out three non-negotiables: (a) the raw `purchaseToken` **stays on-device** — the backend stores `SHA-256(token)` — (b) `expiresAtEpochMs` is the source of truth on liveness (a persisted `isEntitled = true` with an expired timestamp is expired; the honest-numbers check is the caller's job, done in `ProEntitlementRepository` in Slice A4), and (c) writes are always **atomic** — mutate a copy of the current state and hand the whole snapshot back so no torn intermediate is ever observable (e.g. token present but expiry still null). [ProStateJson.kt](../android/app/src/main/java/com/pesatrack/services/pro/ProStateJson.kt) is a tiny top-level `object` that owns a single Moshi instance + `JsonAdapter<ProState>` and exposes `serialize(state)` / `parse(json)`. `parse` catches every Moshi exception and returns `null` on failure so a corrupted or forward-schema blob never wedges the app on cold start — callers substitute `ProState.DEFAULT`. Moshi Kotlin codegen (`moshi-kotlin-codegen` via KSP from Slice A1) generates `ProStateJsonAdapter.kt` under `build/generated/ksp/debug/kotlin/`, verified present after `assembleDebug` — so there's no reflection and R8 stays consumer-rule-free. [AppPreferences.kt](../android/app/src/main/java/com/pesatrack/data/local/preferences/AppPreferences.kt) got a new `KEY_PRO_STATE = stringPreferencesKey("pro_state_json_v1")` (the `_v1` suffix in the key name is the on-disk schema version — a future `_v2` migration reads this key, transforms, writes the new key, deletes the old, all under a single DataStore edit), plus three accessors: a hot `val proState: Flow<ProState>` that emits `DEFAULT` on missing-or-malformed, `suspend fun getProState()` for snapshot reads, and `suspend fun setProState(state: ProState)` for atomic writes. Nine-test [ProStateJsonTest.kt](../android/app/src/test/java/com/pesatrack/services/pro/ProStateJsonTest.kt) covers: default state round-trip, fully-populated monthly round-trip, annual tier preservation, wire-format short-name lock (`"monthly"` / `"annual"` present, `"MONTHLY"` / `"ANNUAL"` **absent** — a rename regression would flip both asserts), malformed-JSON tolerance (`parse` returns `null` for `""`, unquoted-key JSON, garbage), unknown-tier tolerance (a `"tier":"platinum"` payload returns `null` so the caller falls back to `DEFAULT` — this is the primary "corrupted blob never wedges cold start" guarantee), missing-optional-field fallback to data-class defaults, `ProProduct.productId` strings match the backend `KNOWN_PRODUCT_IDS` set (drift check — if these break, `/billing/verify` will 400 on every call), and `fromProductId` inverse. Verified with `./gradlew.bat testDebugUnitTest --tests "com.pesatrack.services.pro.ProStateJsonTest"` (BUILD SUCCESSFUL, all 9 pass), full `./gradlew.bat testDebugUnitTest` (BUILD SUCCESSFUL, no regressions), and `./gradlew.bat assembleDebug` (BUILD SUCCESSFUL, `ProStateJsonAdapter.kt` generated cleanly). No user-visible behaviour change, no manifest change, no persisted state read by any UI yet — Slice A3 wires the HTTP client that will call `/billing/verify` to populate this state. **Slices A3–A6 still ahead on this branch:** Retrofit interfaces + `AiHttpModule` + `ProAuthInterceptor` (A3), `PlayBillingClient` + `ProEntitlementRepository` + `ProPurchaseFlow` (A4), `PesaTrackProScreen` + `ProUpsellSheet` + navigation entry (A5), `pro_ai_enabled` DataStore flag + 7 Pro-lifecycle telemetry events (A6). Version bump `1.5.3 → 1.6.0` (versionCode `16 → 17`) is deferred until A5 lands the UI surface. Website sync check per AGENTS.md §Website Sync Check will trigger at A5 (new screen + new feature) — filed as a follow-up when we get there. Files added by A2: [ProProduct.kt](../android/app/src/main/java/com/pesatrack/services/pro/ProProduct.kt), [ProState.kt](../android/app/src/main/java/com/pesatrack/services/pro/ProState.kt), [ProStateJson.kt](../android/app/src/main/java/com/pesatrack/services/pro/ProStateJson.kt), [ProStateJsonTest.kt](../android/app/src/test/java/com/pesatrack/services/pro/ProStateJsonTest.kt). Files touched: [AppPreferences.kt](../android/app/src/main/java/com/pesatrack/data/local/preferences/AppPreferences.kt). Earlier slice files: [android/app/build.gradle.kts](../android/app/build.gradle.kts), [android/app/src/main/res/xml/network_security_config.xml](../android/app/src/main/res/xml/network_security_config.xml), [android/app/src/main/AndroidManifest.xml](../android/app/src/main/AndroidManifest.xml), [backend/](../backend/) (full new scaffold), [plans/ai-pro-phase1-spec.md](../plans/ai-pro-phase1-spec.md).

- **Play Console fix — remove `AD_ID` permission auto-added by firebase-analytics (v1.5.3)** — Play Console rejected the v1.5.2 review submission with "Incomplete advertising ID declaration" because `firebase-analytics-ktx` transitively depends on `play-services-ads-identifier:18.0.0`, which merges `com.google.android.gms.permission.AD_ID` into the release manifest. PesaTrack does not read the advertising ID anywhere in code (Phase 1 telemetry never touched `AdvertisingIdClient` — that's just what ProGuard's `usage.txt` reflects from the transitive library) and privacy principle #4 (Privacy is non-negotiable) forbids collecting it. Fix is manifest-only: `<uses-permission android:name="com.google.android.gms.permission.AD_ID" tools:node="remove" />` in [AndroidManifest.xml](../android/app/src/main/AndroidManifest.xml) strips the merged permission at build time. Verified by re-checking `build/intermediates/packaged_manifests/release/.../AndroidManifest.xml` after `bundleRelease` (permission must no longer appear). Version bumped `1.5.2 → 1.5.3` (versionCode `14 → 15`). **Play Console follow-up (must do at upload time)**: App content → Advertising ID → answer **"No, my app does not use advertising ID."** Data Safety declaration is unchanged (analytics is still "app activity" with the same collection/sharing scope Phase 1–3 already disclosed). Analytics event delivery and pseudonymous App Instance ID behavior are unaffected — this permission was never being used, only present. Website sync check per AGENTS.md §Website Sync Check: **not required** (no trigger-table match — privacy-policy behavior is unchanged because the removed permission was never actually exercised; `factsheet.json.currentVersion` auto-updates from `_docs/releases.md`). Files: [AndroidManifest.xml](../android/app/src/main/AndroidManifest.xml), [android/app/build.gradle.kts](../android/app/build.gradle.kts), [_docs/releases.md](releases.md).
...
- **Phase 3 telemetry — income, settings, permissions, PIN, and category-edit coverage (v1.5.2)** — Third pass over the opt-in analytics surface, closing the gaps left by Phase 2 ("does anyone use the features we built" → "which controls do they actually touch"). Same collection contract as Phases 1–2: `TelemetryClient.setEnabled(false)` short-circuits every call, only primitives make it into the Firebase `Bundle`, no counterparty text, amounts, category labels, SMS bodies, or user-typed strings leave the device. Version bumped `1.5.1 → 1.5.2` (versionCode `13 → 14`). **Allow-list additions** in [TelemetryEvents.kt](../android/app/src/main/java/com/pesatrack/services/telemetry/TelemetryEvents.kt): income (`income_categorized_manual`, `income_excluded_toggled` with `enabled`, `income_deleted`); settings toggles (`settings_bank_tracking_toggled` with `enabled`, `settings_individual_bank_toggled` with `bank` + `enabled`, `settings_biometric_toggled` with `enabled`, `settings_lock_timeout_changed` with a bucketed `count_bucket`, `settings_month_start_day_changed` with a bucketed `count_bucket`, `settings_categories_reset`); data management (`data_exported`, `database_backed_up`, `database_restored` — each with `success` boolean, `all_data_cleared`); category-management edits (`category_edited` with `kind = group/sub`, `category_rule_edited`, `merchant_recategorized` with `count_bucket`); permissions (`permission_requested`, `permission_granted`, `permission_denied` — all three carry `kind = sms/notification` and `source = onboarding/app`); onboarding funnel (`onboarding_started`, `onboarding_completed` with `kind = import_chosen/skipped`); PIN lock (`pin_setup_completed`, `pin_disabled`, `pin_unlock_failed`); analytics interactions (`analytics_tab_switched` with `tab = insights/charts/monthly/yearly`); expense delete (`expense_deleted`). Added parameter keys `PARAM_ENABLED`, `PARAM_BANK`, `PARAM_SUCCESS`, `PARAM_TAB`, plus new value enums `PERMISSION_SMS`, `PERMISSION_NOTIFICATION`, `SOURCE_ONBOARDING`, `SOURCE_APP`, `ONBOARDING_COMPLETION_IMPORT`, `ONBOARDING_COMPLETION_SKIPPED`, `TAB_INSIGHTS/CHARTS/MONTHLY/YEARLY`. **Two new bucketing helpers** — `timeoutBucket(seconds)` maps auto-lock timeout to `immediate / <=1m / <=5m / <=15m / >15m` and `monthStartDayBucket(day)` maps the budget-cycle start day to `start (1–5) / mid (6–20) / late (21–28) / other`. The month-start bucket exists specifically to protect against back-derivation: for a payday clustering around known days, an outlier's exact day could be semi-identifying, so we never send the raw value. **Income (`CategorizeIncomeViewModel`)** — [CategorizeIncomeViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/categorize_income/CategorizeIncomeViewModel.kt) took `TelemetryClient` via `@Inject`. `save()` emits `income_categorized_manual` after `updateSource` + `setExcluded` + `learnSenderSource` succeed. `toggleExcluded()` emits `income_excluded_toggled` with the *new* value, so a false→true means "user just excluded". `delete()` emits `income_deleted` after `IncomeRepository.delete(id)` returns. Values for `IncomeSource` are intentionally NOT sent — the enum labels are user-visible product copy and would leak the parser's classification. **Settings toggles (`SettingsViewModel`)** — Every `AppPreferences`-write path now emits its matching event, so we can see which controls are actually used vs which are dead UI: master bank-tracking switch, per-bank toggles (safe to send `bankName` because it comes from `SmsParserRegistry.getAllParserNames()` — a hardcoded list, not user input), biometric enable, lock timeout (bucketed), month-start day (bucketed), and "Reset categories to defaults". **Data management** — `exportData`, `backupDatabase`, `restoreDatabase`, `clearAllData` each emit their event with `success = true/false` boolean (where applicable). `database_restored` is emitted **before** the process-kill path (`restartApp`) so the success event actually reaches the wire — otherwise Firebase's async batch upload would race the `Runtime.getRuntime().exit(0)` and drop it. `sample_data_populated` deliberately not tracked — the debug-only affordance shouldn't pollute production dashboards. **Category edits (`CategoryManagementViewModel`)** — `updateCategory` emits `category_edited` with `kind` derived from `allCategories.find { it.id == id }.isGroup`; `updateRule` emits `category_rule_edited`. Phase 2 already handled create/delete; Phase 3 closes the edit branch so the full lifecycle is visible. **Merchant recategorization (`MerchantsViewModel`)** — Got `TelemetryClient` injected. `reassign()` emits `merchant_recategorized` with `count_bucket = countBucket(updated)` after the bulk `UPDATE` + mapping-refresh chain completes. This is the single most sensitive category-quality signal — if this event volume is high, our auto-cat is missing the mark and we should look at recipient mapping heuristics. **Permissions (`MainActivity` + `OnboardingScreen`)** — Requests and outcomes are tracked at *both* entry points with a `source` param to distinguish onboarding-primer flow from later re-asks. `MainActivity.permissionLauncher` (used by `requestNotificationPermission` on Android 13+ and `requestSmsPermissionsIfNeeded` for returning users) now emits `permission_requested` before `.launch(...)` and `permission_granted`/`permission_denied` per-permission from the result callback, all with `source = app`. Permissions we don't classify (nothing in the current app, but future-proofing) get their `kind` resolved to `null` and are silently dropped — no novel enum values ever leak. `OnboardingScreen` composable gained `onSmsPermissionRequested` and `onSmsPermissionDenied` callbacks; the primer button now calls `onSmsPermissionRequested()` before `smsPermissionLauncher.launch(...)`, and the launcher's result callback splits into granted vs denied paths. `MainActivity.AppEntryPoint` wires these to `permission_requested`/`granted`/`denied` with `source = onboarding` so the onboarding-primer grant rate is directly comparable to later app-level asks — that's the primary product question the primer pattern is meant to answer. **Onboarding funnel** — `AppEntryPoint.LaunchedEffect(Unit)` emits `onboarding_started` alongside the existing `recordOnboardingStarted()` milestone write. `OnboardingScreen.onComplete` callback in MainActivity now derives the completion kind from `pendingImportNavigation` (the same boolean that decides `recordOnboardingImportSkipped` vs the earlier `recordOnboardingImportChosen`) and emits `onboarding_completed` with `kind = import_chosen` or `skipped`. That's enough to see the import-conversion rate without adding a second event. `import_chosen` also implicitly connects to the Phase 2 `import_started`/`completed` events on the ImportViewModel side, so we can follow the whole "onboarding → import → categorization" chain end-to-end. **PIN lock (`PinViewModel`)** — Got `TelemetryClient` injected. `setupConfirm` and `changeConfirm` both emit `pin_setup_completed` on the success branch (after `pinManager.savePin(pin)`) — the two paths share one event because the intent (a working PIN now exists) is identical. `verifyDisable` emits `pin_disabled` on the success branch after `pinManager.clearPin()`. `handleWrongPin` emits `pin_unlock_failed` on every wrong-PIN attempt across all verification flows (unlock, change, disable). Success on unlock is *not* emitted — it can be derived from the ratio of failures to `app_opened`, so the extra event would be redundant. Attempt-number sequence intentionally NOT sent to avoid enabling brute-force pattern fingerprinting. **Analytics interactions (`AnalyticsViewModel`)** — Got `TelemetryClient` injected. `selectTab` and `selectInsightsTab` emit `analytics_tab_switched` with `tab` mapped from the `AnalyticsTab` / `InsightsTab` enums to the small allow-listed value set. One event covers both nesting levels because the product question ("which content are people actually reading in Analytics?") is the same. **Expense delete (`ExpensesViewModel`)** — Got `TelemetryClient` injected. `deleteExpense` emits `expense_deleted` after the repository delete completes; no params (no amount, no recipient, no category — same discipline as `expense_manual_added`). **Deliberately still not instrumented** — expense edit (long-tail signal, low product-decision value), `sample_data_populated` (debug affordance), income-period navigation (subsumed by `screen_viewed`), review screens (subsumed by `screen_viewed` route bases `weekly_review`/`monthly_review`/`quarterly_review`/`year_in_review`), PIN unlock *successes* (derivable from the failure ratio), individual notification-cadence pref toggles (would need their own bucketing pass — deferred). Design constraint continues to hold: *if the event does not answer a specific product-team question, do not add it.* Verified with `./gradlew :app:assembleDebug` (BUILD SUCCESSFUL in 2m 30s) and `:app:test` (BUILD SUCCESSFUL in 1m 56s). **Play Console Data Safety declaration remains the shipping blocker** — the new events are still "app activity" (feature taps, permission outcomes, PIN lifecycle) which is the same disclosure Phase 1 & 2 already required. No new categories to declare. Files: [TelemetryEvents.kt](../android/app/src/main/java/com/pesatrack/services/telemetry/TelemetryEvents.kt), [MainActivity.kt](../android/app/src/main/java/com/pesatrack/presentation/MainActivity.kt), [OnboardingScreen.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/onboarding/OnboardingScreen.kt), [CategorizeIncomeViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/categorize_income/CategorizeIncomeViewModel.kt), [SettingsViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/settings/SettingsViewModel.kt), [CategoryManagementViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/category_management/CategoryManagementViewModel.kt), [MerchantsViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/merchants/MerchantsViewModel.kt), [PinViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/pin/PinViewModel.kt), [AnalyticsViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/analytics/AnalyticsViewModel.kt), [ExpensesViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/expenses/ExpensesViewModel.kt), [android/app/build.gradle.kts](../android/app/build.gradle.kts).

- **Phase 2 telemetry — engagement event taxonomy (v1.5.1)** — Follows Phase 1 by shipping the actual event surface behind the same opt-in consent flag. Nothing about the collection contract changes — `TelemetryClient.setEnabled(false)` still short-circuits every call, only primitives make it into the Firebase `Bundle`, and none of the new events carry counterparty names, amounts, category labels, SMS bodies, or notification-content strings. Version bumped `1.5.0 → 1.5.1` (versionCode `12 → 13`). **New event allow-list** — added to [TelemetryEvents.kt](../android/app/src/main/java/com/pesatrack/services/telemetry/TelemetryEvents.kt) alongside the Phase 1 consent set: `screen_viewed` (param `screen` = route base with args stripped), `sms_parsed` (`source` = parser display name like `M-PESA` / `NCBA`, `kind` = `expense`/`income`), `expense_categorized_manual`, `expense_categorized_batch` (`count_bucket`), `expense_manual_added`, `import_started` / `import_completed` / `import_failed` (`source` = `sms`/`excel`/`statement_pdf`, plus `count_bucket` on completed), `budget_saved` (`scope` = `overall`/`category`/`income`), `category_created` (`kind` = `group`/`sub`) / `category_deleted`, `category_rule_created` / `category_rule_deleted`, `notification_opened` (`kind` = `income`/`categorize`/`review`/`budget`). Same file also declares every parameter key (`PARAM_*`) and every legal value enum as a `const val` so any future emitter has to reference the allow-list rather than string-literal something novel. **Bucketing helper** — `TelemetryEvents.countBucket(n)` returns `"0" / "1-10" / "11-100" / "101-1k" / "1k+"`, applied to every count-carrying event (import volumes, batch categorize sizes) so a rare precise number (like "imported 4,731 expenses") can't be back-derived to identify one user. Exact counts never leave the device. **Screen tracking** — `MainScreen` composable in [MainActivity.kt](../android/app/src/main/java/com/pesatrack/presentation/MainActivity.kt) took a new `onScreenViewed: (String) -> Unit` callback and now runs a `LaunchedEffect(currentDestination?.route)` that strips both `?` (`analytics?section={section}` → `analytics`) and `/` (`categorize/{expenseId}` → `categorize`) before emitting, so no argument value (expense id, snapshot id, section deep-link) can ever be sent — only the route base. Wired at the call site inside `AppWithLockOverlay` so it fires only after PIN unlock, meaning locked-app time doesn't pollute engagement metrics. Review screens (weekly/monthly/quarterly/year-in-review) are covered by the same `screen_viewed` event with distinct route bases, so a separate `review_opened` was deliberately omitted from the taxonomy — one signal, not two. **SMS parsing (health metric)** — [SmsReceiver.kt](../android/app/src/main/java/com/pesatrack/services/SmsReceiver.kt) received a `@Inject TelemetryClient` and now emits `sms_parsed` once for each expense or income row that is persisted (after the dedupe check, so retried duplicates don't double-count). Source is resolved via `SmsParserRegistry.findParser(sender, body)?.displayName ?: "other"` — the raw sender address never leaves the device. `handleExpenseResult` / `handleIncomeResult` gained a `senderAddress` parameter for this purpose only. This gives us a Play Store parse-success signal that the existing `AppPreferences.incrementSmsParsedCount()` counter can't (that one is only visible to the user in-app). **Imports (start / complete / fail funnel)** — Three ViewModels now emit `import_started` before the coroutine launches and either `import_completed` (with a bucketed row count) or `import_failed` in the catch block: [ImportViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/import_history/ImportViewModel.kt) for historical SMS (`source = sms`, count = `newExpensesImported + newIncomesImported`), [ExcelImportViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/excel_import/ExcelImportViewModel.kt) for `.xlsx` (`source = excel`, count = `rowsImportedAsStandalone + rowsMatchedToSms`), [StatementImportViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/statement_import/StatementImportViewModel.kt) for M-PESA PDF statements (`source = statement_pdf`, count = `result.imported`). PDF import also fires `import_failed` on the partial-error branch (`result.error != null && result.imported == 0`) as well as the try/catch, so parse failures are visible even when the file "opened" successfully. **Categorization funnel** — [CategorizeViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/categorize/CategorizeViewModel.kt) emits `expense_categorized_manual` after `expenseRepository.updateCategory` succeeds. [BatchCategorizeViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/batch_categorize/BatchCategorizeViewModel.kt) emits `expense_categorized_batch` from all four apply paths (quick-mode single-group, individual review-mode override, apply-all auto-suggestions, bulk multi-select apply) with `count_bucket` reflecting the number of expenses actually touched. Single-expense-in-batch paths bucket to `"1-10"` explicitly. **Manual entry** — [ManualEntryViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/manual_entry/ManualEntryViewModel.kt) emits `expense_manual_added` on save success. No amount, no recipient, no category — the event exists just to answer "is anyone actually using this feature?". **Budget engagement** — [BudgetViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/budget/BudgetViewModel.kt) emits `budget_saved` from both `saveBudget()` (`scope = category`) and `saveIncome()` (`scope = income`); the `overall` scope enum value is reserved for a future "total spending cap" feature so no code path emits it today. Deletes are not tracked yet — deferred to a Phase 3 signal-review pass once we see whether the create rate is meaningful at all. **Category management** — [CategoryManagementViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/category_management/CategoryManagementViewModel.kt) emits `category_created` (`kind = group` or `sub`), `category_deleted` (only when the DAO reports `success` — declined-because-still-has-expenses attempts are silent), `category_rule_created` and `category_rule_deleted`. Update events are not tracked; only create/delete carry meaningful lifecycle signal. **Notification tap attribution** — [MainActivity.handleDeepLinkIntent](../android/app/src/main/java/com/pesatrack/presentation/MainActivity.kt) buckets the incoming `navigate_to` extra into `categorize` (single-expense + income-categorize), `review` (weekly/monthly/quarterly/year), or `budget` and emits `notification_opened`. Unrecognised targets emit nothing rather than a novel enum value. Runs on both `onCreate`-with-intent and `onNewIntent`, so relaunches from notifications while the app is warm still count. **Not instrumented (intentional)** — expense delete, expense edit, income exclude/delete, PIN enable/disable, biometric fallback, onboarding step progress, first-open. Onboarding funnel is the highest-value gap; deferred to a follow-up so this PR stays focused on "does anyone use the features we built" rather than "do people finish the tour". Design constraint continues to be: *if the event does not answer a specific product-team question, do not add it.* Verified with `./gradlew :app:assembleDebug` (BUILD SUCCESSFUL in 2m 8s) and `:app:test` (BUILD SUCCESSFUL in 1m 51s). Files: [TelemetryEvents.kt](../android/app/src/main/java/com/pesatrack/services/telemetry/TelemetryEvents.kt), [MainActivity.kt](../android/app/src/main/java/com/pesatrack/presentation/MainActivity.kt), [SmsReceiver.kt](../android/app/src/main/java/com/pesatrack/services/SmsReceiver.kt), [ImportViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/import_history/ImportViewModel.kt), [ExcelImportViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/excel_import/ExcelImportViewModel.kt), [StatementImportViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/statement_import/StatementImportViewModel.kt), [CategorizeViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/categorize/CategorizeViewModel.kt), [BatchCategorizeViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/batch_categorize/BatchCategorizeViewModel.kt), [ManualEntryViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/manual_entry/ManualEntryViewModel.kt), [BudgetViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/budget/BudgetViewModel.kt), [CategoryManagementViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/category_management/CategoryManagementViewModel.kt), [android/app/build.gradle.kts](../android/app/build.gradle.kts).

- **Phase 1 telemetry — Firebase Analytics behind opt-in consent (v1.5.0)** — Ended PesaTrack's absolute-offline stance in exchange for measurable user behaviour instrumentation. This is a deliberate policy shift (previously the About screen + privacy policy claimed "no internet permission — PesaTrack cannot send data anywhere"). Phase 1 ships plumbing + consent only; transactions, SMS content, amounts, merchants, and PII still never leave the device. **Manifest:** `android.permission.INTERNET` + `ACCESS_NETWORK_STATE` added. Firebase Analytics collection is default-disabled at manifest level via `firebase_analytics_collection_enabled=false` plus all four `google_analytics_default_allow_*` flags = `false`, so the SDK cannot collect anything until we explicitly opt in from code — belt-and-braces for the Play Store Data Safety declaration. **Gradle:** top-level `com.google.gms.google-services:4.4.2` declared `apply false`; app-level applies it conditionally only when `google-services.json` exists so local/CI builds without the config file still succeed. Firebase BOM `33.7.0` + `firebase-analytics-ktx`. Version bumped `1.4.1 → 1.5.0` (versionCode `11 → 12`). `google-services.json` added to `.gitignore`. **Vendor-agnostic client** — new `services/telemetry/TelemetryClient` interface + `FirebaseTelemetryClient` (release) + `NoOpTelemetryClient` (debug + fallback when Firebase init fails). All emission goes through the interface — the app never calls Firebase directly, so swapping vendors is a one-file change. `setEnabled(false)` immediately calls `FirebaseAnalytics.setAnalyticsCollectionEnabled(false)` **and** `resetAnalyticsData()` so the previous session's pseudonymous App Instance ID cannot be linked to future data. `logEvent(name, params)` short-circuits when disabled and only accepts primitive params (arbitrary object values are silently dropped so PII can't be smuggled through `toString()`). **Allow-list** — `TelemetryEvents` object declares the only strings that may be sent in Phase 1: `app_opened`, `telemetry_prompt_shown`, `telemetry_prompt_dismissed`, `telemetry_enabled`, `telemetry_disabled`. Any future event must land in this file. **Persistence** — new DataStore keys `telemetry_enabled` (default `false`) + `telemetry_prompt_shown` (default `false`) on `AppPreferences`, with `telemetryEnabled` / `telemetryPromptShown` Flows, `isTelemetryEnabled()` / `wasTelemetryPromptShown()` snapshots, and `setTelemetryEnabled(bool)` / `markTelemetryPromptShown()` setters. `PesaTrackApp.onCreate` applies the persisted flag to `TelemetryClient` on cold start so collection state survives process restarts. **Consent UX** — new `TelemetryConsentSheet` (ModalBottomSheet) shown once, only after onboarding completes and the PIN unlock overlay is dismissed, gated by `!telemetryPromptShown`. Designed to bias toward acceptance without dark patterns per AGENTS.md "Nudge, don't nag": primary CTA is a benefit-framed filled button ("Yes, help improve PesaTrack") not a consent verb; secondary "Not now" is a plain `TextButton` (not styled as a warning) so declining is low-friction, not shameful; two info cards make the collection scope concrete — a short "What we collect" list (screens opened, feature taps, app version, crash signals) sits above a longer "What never leaves your phone" list (SMS content, amounts, merchants, account numbers, name/phone/email); footer reads "You can change this any time in Settings" + a link to the full policy. Both buttons mark the sheet as shown permanently — the Settings toggle is the ongoing control, no re-nagging. Emits `telemetry_prompt_shown` on display, `telemetry_enabled` after `setEnabled(true)` on accept (so it's the first event of the newly-consented session), `telemetry_prompt_dismissed` on decline. **Settings toggle** — new `PrivacyAnalyticsSection` between Data Management and About, wired through `SettingsViewModel.setTelemetryEnabled(bool)`; on disable, the `telemetry_disabled` event is sent *before* collection turns off so it actually reaches the wire. Subtitle mirrors the current state honestly: "On — thanks for helping improve PesaTrack" / "Off — nothing is transmitted from your device". **Copy updates** — About screen data-practices card and hero subtitle rewritten to separate on-device data from opt-in telemetry ("Your transactions stay on your device" instead of "All data stays on your device"; added bullet "Anonymous usage analytics are opt-in and can be turned off in Settings"). **Privacy policy** — `docs/privacy-policy.html` fully rewritten: new §4 "Anonymous Usage Analytics (Opt-In)" with explicit collected / never-collected lists and disable instructions, §5 rewritten so the "never shared" claim now specifically covers transaction data (not all data), §5.6 lists Google Play Services in use, §7 adds "Disable analytics" and "Reset analytics ID" to user rights, and the highlight callout at the top now leads with the new invariant: "Your transactions stay on your device. The only data that ever leaves your phone is anonymous usage analytics — and only if you explicitly opt in." Last-updated date bumped to Sep 3, 2026. **Debug/release split** — `TelemetryModule` returns `NoOpTelemetryClient` when `BuildConfig.DEBUG` so local dev never emits events; also returns no-op if `FirebaseTelemetryClient` init throws (missing config) rather than crashing. **Setup requirement for release builds** — provision a Firebase project, download `google-services.json`, drop it in `android/app/`. Without the file, release builds still compile (the plugin is conditional) but events never reach Firebase. **Not shipped in this PR (deferred):** Play Store Data Safety declaration must be updated in Play Console **before** the 1.5.0 rollout — analytics collection must be disclosed as "app activity" data type shared with Google. See [plans/cloud-sync-playstore-impact.md](../plans/cloud-sync-playstore-impact.md) for the process; the same Data Safety mechanics apply. Verified with `./gradlew :app:assembleDebug` and `:app:test` (both success). Files: [AndroidManifest.xml](../android/app/src/main/AndroidManifest.xml), [android/build.gradle.kts](../android/build.gradle.kts), [android/app/build.gradle.kts](../android/app/build.gradle.kts), [android/.gitignore](../android/.gitignore), [TelemetryEvents.kt](../android/app/src/main/java/com/pesatrack/services/telemetry/TelemetryEvents.kt), [TelemetryClient.kt](../android/app/src/main/java/com/pesatrack/services/telemetry/TelemetryClient.kt), [NoOpTelemetryClient.kt](../android/app/src/main/java/com/pesatrack/services/telemetry/NoOpTelemetryClient.kt), [FirebaseTelemetryClient.kt](../android/app/src/main/java/com/pesatrack/services/telemetry/FirebaseTelemetryClient.kt), [TelemetryModule.kt](../android/app/src/main/java/com/pesatrack/di/TelemetryModule.kt), [TelemetryConsentSheet.kt](../android/app/src/main/java/com/pesatrack/presentation/components/TelemetryConsentSheet.kt), [AppPreferences.kt](../android/app/src/main/java/com/pesatrack/data/local/preferences/AppPreferences.kt), [PesaTrackApp.kt](../android/app/src/main/java/com/pesatrack/PesaTrackApp.kt), [MainActivity.kt](../android/app/src/main/java/com/pesatrack/presentation/MainActivity.kt), [SettingsUiState.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/settings/SettingsUiState.kt), [SettingsViewModel.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/settings/SettingsViewModel.kt), [SettingsScreen.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/settings/SettingsScreen.kt), [AboutScreen.kt](../android/app/src/main/java/com/pesatrack/presentation/screens/about/AboutScreen.kt), [docs/privacy-policy.html](../docs/privacy-policy.html).
